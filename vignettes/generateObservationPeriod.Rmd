...
For all the definitions of observation period we will use `censorAge = 120` and `censorDate = as.Date("2020-01-01")`. Note also that if a death record is in the database the person will automatically be censored that day.

## First to extraction

To built observation period that span from the first record to the data extraction we will need to set the `collapseEra` and `persistenceWindow` to `Inf` as we want to span the observation period till the extraction of data.
...
TODO: Invite collaborators to submit their results and have a shiny with the comparison and impact of the decision in different datasets.
