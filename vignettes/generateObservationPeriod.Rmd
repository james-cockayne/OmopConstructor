...
5.  *Collapse+Persistence 180*: individuals are in observation while they have an ongoing record, different records are collapsed using a window of 180 days, a persistence window of 180 days is added at the end of each observation period.
6.  *Collapse+Persistence 365*: individuals are in observation while they have an ongoing record, different records are collapsed using a window of 365 days, a persistence window of 365 days is added at the end of each observation period.
...
To collapse records separated by 180 days we will use the `collapseEra` argument (`collapseEra = 180`), this time we will set `persistenceWindow` to `180` as well as we want to add 180 days after any record.
...
In this case we will set both arguments to `365` analogously to the previous definition.
